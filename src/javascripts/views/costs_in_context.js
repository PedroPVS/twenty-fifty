window.twentyfifty.views.costs_in_context = function () {
    ...
    this.setup = function () {
        ...
        this.r.text(this.x(0), this.h - 5, "O custo absoluto para a sociedade de todo o sistema de energia").attr({
            'text-anchor': 'start',
            'font-weight': 'bold',
            'fill': '#008000'
        });
        ...
        this.drawIndicator(3700, "Custo aproximado do sistema de energia nos dias atuais");
        _results = [];
    ...
    this.updateBar = function (pathway, _id) {
        ...
        if (pathway.ghg_reduction_from_1990 < 0.8) {
            bar.message.attr({
                ...
                text: "Esta via não reduz as emissões em pelo menos 80% nos níveis de 1990"
            });
    ...
    this.setIncrementalCostLabel = function (code, pathway) {
        ...
        if (pathway.total_cost_range_adjusted === 0 && this.comparator.total_cost_range_adjusted === 0) {
            ...
            if (i1 < 0 && i2 < 0) {
                message = "" + (-i1) + " até " + (-i2) + " menos";
            } else if (i1 < 0 && i2 > 0) {
                message = "" + (-i1) + " menos até " + i2 + " mais";
            } else {
                message = "" + i1 + " até " + i2 + " mais";
            }
        }
