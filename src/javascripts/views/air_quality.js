window.twentyfifty.views.air_quality = function () {
    ...
    var margin = { top: 40, right: 30, bottom: 30, left: 300 },
        ...
        svg,
        bars,
        data = [{ name: '2013', caption: '2013', css: 'today', low: 100, high: 100 }];
    ...
    this.setup = function () {
        ...
        svg.append("text")
          ...
          .attr("class", "caption")
          .text("Índice de impacto na saúde por poluição do ar (2013=100)");
    ...
    this.updateComparator = function (pathway) {
        data[1] = {
            name: "comparator",
            css: "comparator",
            caption: "2050 - " + twentyfifty.pathwayName(pathway['_id'], "Comparação"),
    ...
    this.updateResults = function (pathway) {
        data[1] = {
            name: "chosen",
            css: "chosen",
            caption: "2050 - " + twentyfifty.pathwayName(pathway['_id'], "Sua definição"),
            low: pathway.air_quality[1][1],
            high: pathway.air_quality[0][1]
        }
        updateMessage();
    ...
    updateMessage = function () {
        ...
        if (high > 100) {
            d3.select("#airquality_range_start").text(amount + " maior até ");
        } else if (high < 100) {
            d3.select("#airquality_range_start").text(amount + " menor até ");
        } else {
        ...
        if (low > 100) {
            d3.select("#airquality_range_end").text(amount + " maior");
        } else if (low < 100) {
            d3.select("#airquality_range_end").text(amount + " menor");
        } else {
